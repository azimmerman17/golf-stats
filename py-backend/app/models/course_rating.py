...
class Course_Rating(db.Model):
  ...
  hole_count = db.Column(db.Integer, db.CheckConstraint('hole_count = 9 OR hole_count = 18', name='rating_hole_count'), server_default='18')
  gender = db.Column(db.Enum('M','F', name='course_rating_gender'), nullable=False, server_default='M')
  start_hole = db.Column(db.Integer, db.CheckConstraint('start_hole >= 1 AND start_hole <= 18', name='rating_start_hole'), nullable=False, server_default='1')
  course_rating = db.Column(db.FLOAT, nullable=False)
  slope = db.Column(db.Integer, db.CheckConstraint('slope >= 55 AND slope <= 155', name='rating_slope'), nullable=False)
  par = db.Column(db.Integer, db.CheckConstraint('par >= 27 AND par <= 80', name='rating_par'), nullable=False)
