...
class Hole(db.Model):
  ...
  gender = db.Column(db.Enum('M','F', name='hole_gender'), nullable=False, server_default='M')
  ...
  def __init__(self, hole_id, tee_id=None, number=None, gender=None, yards=None, meters=None, par=None, si=None, effective_date=None, created_at=None, updated_at=None):
  ...
  def insert_row(self, data):
    ...
    for k in data.keys():
      ...
      values =  f"{values}{'' if values == '' else ', '}'{data[k]}'"
      conflict = f"{conflict}, {k} = '{data[k]}'"

    query = f"""
    INSERT INTO "Hole" ({keys})
    VALUES ({values})
    ON CONFLICT (hole_id) DO UPDATE 
    SET
      updated_at = NOW()
      {conflict};
    ;"""
